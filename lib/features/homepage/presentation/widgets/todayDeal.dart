...
class _TodayDealState extends State<TodayDeal> {
  ...
  int selectedIndex = 0;
  List<String> tab = [];
  List<TodaysDealsEntity> filterPro = [];
  List<ProductEntity> pro = [];

  @override
  Widget build(final BuildContext context) {
    return BlocBuilder<HomePageTodaysdealsBloc, HomePageTodaysdealsState>(
      builder: (context, state) {
        if (state is HomePageTodaysdealsLoading) {
          return Center(
            child: CircularProgressIndicator(),
          );
        }
        if (state is HomePageTodaysdealsError) {
          return Center(
            child: Text(state.error.toString()),
          );
        }
        if (state is HomePageTodaysdealsLoaded) {
          if (tab.isEmpty) {
            for (final i in state.todaysdeals!) {
              tab.add(i.tab!);
            }
          }
